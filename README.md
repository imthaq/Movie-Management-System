...
Search by title - Retrieve detailed stats (rating, duration, cast, genres) using an exact string match
Search by actor - List all stored movies featuring a specific actor
Search by genre - View a complete list of movies tagged with a specific genre
Search by year - Find all movies released in a given year
Search by rating range - Filter movies by providing a minimum and maximum rating (e.g., 8.0 to 10.0)
Recommend movies - Input a movie title to receive a BFS-generated list of similar films based on shared cast and genres
Shortest connection path - Enter two movie titles to trace the shortest path connecting them through shared metadata
Show recently viewed - View your local search history for the current session
Show co-actors - Input an actor's name to generate a dynamic list of every other actor they have worked with in the dataset
Analyze director filmography - Input a director's name to see their career span, average film rating, and total films
Exit - Safely terminate the program and free all allocated memory
...
This project was developed as part of the Data Structures semester project, demonstrating proficiency in algorithm design, memory management, and low-level C++ programming.
